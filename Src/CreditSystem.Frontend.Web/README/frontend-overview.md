...
| `/desembolsos` | `Desembolsos/Listado.razor` | `GET loans/pending-disbursement`, `POST loans/{id}/confirm-disbursement`, `POST loans/{id}/fail-disbursement` |
...
| `/admin/proyecciones` | `Admin/Proyecciones.razor` | `GET admin/projection/failures`, `POST admin/projection/failures/{id}/resolve`, `POST admin/projection/rebuild` |
| `/tasas` | `Configuracion/Tasas.razor` | `GET/PUT reference-rates/{id}` |
| `/politicas` | `Configuracion/Politicas.razor` | `GET underwriting-policies` |
| `/webhooks` | `Configuracion/Webhooks.razor` | `POST webhooks/subscribe` |

Cobertura: **58/58 endpoints del `open_api.json` original (54 rutas) + 7 endpoints agregados después por el backend (61 rutas totales) tienen pantalla.**

### Endpoints agregados al backend después del `open_api.json` sincronizado

El backend real (`Backend/Credito/Src/Core/CreditSystem.Api/EndPoints/`) creció por encima del `open_api.json` sincronizado (54 rutas). Se revisó código fuente contra código fuente (no hay swagger actualizado que regenerar) para los 7 endpoints nuevos: `ILoanService` se extendió con la cola de desembolsos, y se agregaron `IUnderwritingPolicyService` e `IProjectionAdminService` como servicios nuevos — cada uno con su modelo (`Models/Requests|Responses`), su página (`Desembolsos/Listado.razor`, `Configuracion/Politicas.razor`, `Admin/Proyecciones.razor`) y su enlace en `AppSidebar`.

Sus modelos tienen el comentario `<auto-generated>` apuntando al archivo fuente de `CreditSystem.Api`/`CreditSystem.Application` en vez de al swagger. Cuando se regenere `open_api.json` desde el backend real, resincronizar esos 9 archivos contra el swagger y actualizar esta nota.

### Módulo CRM (Crm.WebApi, tag "RiskEngine")

Primer módulo del frontend que consume un swagger distinto (`openapi_crm.txt`, backend `Crm.WebApi`) en vez de `open_api.json`. Cubre solo el tag `RiskEngine` (mantenimiento de reglas y matrices de riesgo, más una pantalla de prueba de evaluación) — los demás tags del CRM (`Prospects`, `Customers`, `CreditApplications`, `Workflow`, etc.) no tienen pantalla todavía.

| Ruta | Página | Endpoints del swagger CRM |
|---|---|---|
| `/crm/riesgo/reglas` | `Crm/Riesgo/ReglasListado.razor` | `GET risk-rules` |
| `/crm/riesgo/reglas/nuevo` | `Crm/Riesgo/ReglaNueva.razor` | `POST risk-rules` |
| `/crm/riesgo/reglas/{id}` | `Crm/Riesgo/ReglaDetalle.razor` | `GET/PUT/DELETE risk-rules/{id}` |
| `/crm/riesgo/matrices` | `Crm/Riesgo/MatricesListado.razor` | `GET risk-matrices` |
| `/crm/riesgo/matrices/nuevo` | `Crm/Riesgo/MatrizNueva.razor` | `POST risk-matrices` |
| `/crm/riesgo/matrices/{id}` | `Crm/Riesgo/MatrizDetalle.razor` | `GET/PUT/DELETE risk-matrices/{id}`, `POST risk-matrices/{id}/activate` |
| `/crm/riesgo/evaluaciones` | `Crm/Riesgo/EvaluacionPrueba.razor` | `POST risk-evaluations` |

`IRiskEngineService` (no confundir con `IRiskService`, que es el grupo "Risk Classification" de `CreditSystem.Api`) reutiliza el mismo `IApiClient`/`HttpClient` "CreditSystemApi" y el mismo `Api:BaseUrl`/`ApiKey` que el resto de la app — **DOCUMENTACION INSUFICIENTE**: se asume que `Crm.WebApi` está detrás del mismo gateway; si no es así, hará falta una configuración/`HttpClient` propia solo para este módulo.

`IApiClient` se extendió con `DeleteAsync` (antes no existía ningún endpoint `DELETE` en el frontend) — implementado en `ApiClient` (`HttpClient.DeleteAsync`) y en `DummyApiClient` (mismo patrón que `PostAsync`/`PutAsync` sin body).
...
| Catálogo y casing de `ruleType`/`targetField` de reglas de riesgo (`RiskRuleType`, `RiskRuleTargetField`) | `Models/Enums/RiskRuleType.cs`, `Models/Enums/RiskRuleTargetField.cs` | El swagger CRM expone ambos como `string` libre sin `enum`. Se modelaron como enums C# (con `JsonStringEnumConverter`) a pedido explícito, con la expectativa de migrar a catálogos reales del backend más adelante |
| Catálogo y casing de `status` de matriz de riesgo (`RiskMatrixStatus`) | `Models/Enums/RiskMatrixStatus.cs` | Igual que arriba; se infirió `Draft`/`Active`/`Archived` de que existe `POST .../activate` y un campo `version` |
| Si `Crm.WebApi` está detrás del mismo gateway/`BaseUrl`/`ApiKey` que `CreditSystem.Api` | `Services/Api/RiskEngineService.cs` | Se asume que sí (mismo `IApiClient`); si no, hace falta una configuración/`HttpClient` propia solo para el módulo CRM |
